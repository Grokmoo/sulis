...
function apply_effect(parent, ability, targets)
  ...
  local amount = 10 + stats.caster_level / 2 + stats.wisdom_bonus / 4
  ...
  local anim = target:create_particle_generator("arcane_ball")
  anim:set_moves_with_parent()
  anim:set_position(anim:param(-1.0), anim:param(-0.5))
  ...
  anim:set_particle_position_dist(anim:dist_param(anim:uniform_dist(-1.0, 1.0), anim:uniform_dist(-0.1, 0.1)),
                                  anim:dist_param(anim:fixed_dist(0.0), anim:uniform_dist(-1.5, -2.0)))
