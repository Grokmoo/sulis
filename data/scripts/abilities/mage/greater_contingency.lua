...
function spell_select(parent, ability, targets, selection)
  parent:set_flag("__gr_contingency_trigger_condition", selection:value())

  local cb = ability:create_callback(parent)
  cb:set_on_menu_select_fn("create_contingency")
  
  local abilities = { "healing_circle", "ward", "chameleon", "restoration", "luck" }
...
function create_contingency(parent, ability, targets, selection)
  parent:set_flag("__gr_contingency_spell", selection:value())
  ...
  anim:set_position(anim:param(-2.0), anim:param(-0.6))
  anim:set_particle_size_dist(anim:fixed_dist(4.0), anim:fixed_dist(2.0))
...
function fire_contingency(parent, ability)
  local selection = parent:get_flag("__gr_contingency_spell")

  ability:deactivate(parent)
  game:say_line("Greater Contingency", parent)
...
function get_trigger_condition(parent)
  return parent:get_flag("__gr_contingency_trigger_condition")
end

function on_removed(parent)
  parent:clear_flag("__gr_contingency_trigger_condition")
  parent:clear_flag("__gr_contingency_spell")
end
