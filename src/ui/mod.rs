...
pub fn create_ui_tree<'a>(area_state: Rc<RefCell<AreaState<'a>>>,
    ...
    root.state.set_background(ResourceSet::get_image("background"));
    setup_widgets(&mut root, area_state);

    root
}
fn setup_widgets<'a>(root: &mut Widget<'a>, area_state: Rc<RefCell<AreaState<'a>>>) {
    ...
    let right_pane_x = root.state.inner_right() - right_pane_width;
    ...
    mouse_over.borrow_mut().state
        .set_position(right_pane_x, root.state.inner_position.y);

    let mut button = Widget::with_position(
            Button::new(Box::new(|_w, _s| trace!("Hello world"))),
            Size::new(right_pane_width - 1, 3),
            Point::new(right_pane_x, root.state.inner_position.y + 2));
    button.state.set_text("Test");
    button.state.set_background(ResourceSet::get_image("background"));
