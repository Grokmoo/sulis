...
impl WidgetKind for SaveWindow {
    ...
    fn on_add(&mut self, _widget: &Rc<RefCell<Widget>>) -> Vec<Rc<RefCell<Widget>>> {
        ...
        let id_box = Widget::empty("id"); {
            ...
            let field = Widget::with_defaults(InputField::new(&self.area_editor.borrow().model.id()));

            let area_editor_ref = Rc::clone(&self.area_editor);
            field.borrow_mut().state.add_callback(Callback::new(Rc::new(move |_, kind| {
        ...
        let name_box = Widget::empty("name"); {
            ...
            let field = Widget::with_defaults(InputField::new(&self.area_editor.borrow().model.name()));

            let area_editor_ref = Rc::clone(&self.area_editor);
            field.borrow_mut().state.add_callback(Callback::new(Rc::new(move |_widget, kind| {
                let input_field = match kind.as_any_mut().downcast_mut::<InputField>() {
                    Some(input_field) => input_field,
                    None => panic!("Failed to downcast to InputField"),
                };
        ...
        let filename_box = Widget::empty("filename"); {
            ...
            let field = Widget::with_defaults(InputField::new(&self.area_editor.borrow().model.filename()));
        ...
        let loc_box = Widget::empty("location_kind"); {
            for kind in LocationKind::iter() {
                ...
                button.borrow_mut().state.add_text_arg("name", &format!("{:?}", kind));
        ...
        let on_rest_box = Widget::empty("on_rest"); {
            ...
            disabled.borrow_mut().state.add_callback(Callback::new(Rc::new(move |widget, _| {
                area_editor_ref.borrow_mut().model.on_rest =
                    OnRest::Disabled { message: "<<PLACEHOLDER>>".to_string() };
                widget.borrow_mut().state.set_active(true);
            ...
            fire_script.borrow_mut().state.add_callback(Callback::new(Rc::new(move |widget, _| {
        ...
        let vis_dist_box = Widget::empty("vis_dist"); {
            Widget::add_child_to(&vis_dist_box, Widget::with_theme(Label::empty(), "vis_dist_label"));
            Widget::add_child_to(&vis_dist_box, Widget::with_theme(Label::empty(), "vis_dist_up_label"));
            ...
            let area_editor_ref = Rc::clone(&self.area_editor);
            vis_dist.borrow_mut().state.add_callback(Callback::new(Rc::new(move |_, kind| {
            ...
            let area_editor_ref = Rc::clone(&self.area_editor);
            up_vis_dist.borrow_mut().state.add_callback(Callback::new(Rc::new(move |_, kind| {
                let spinner = match kind.as_any_mut().downcast_mut::<Spinner>() {
                    Some(widget) => widget,
                    None => panic!("Failed to downcast to Spinner"),
                };
