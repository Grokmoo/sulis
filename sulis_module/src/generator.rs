...
pub fn generate_area(
    ...
    let tiles: Vec<Rc<Tile>> = module
        .tiles
        .iter()
        .filter(|&(_, tile)| tile.width == 1 && tile.height == 1)
        .map(|(_, tile)| Rc::clone(tile))
        .collect();

    let (passable_tiles, impassable_tiles): (Vec<Rc<Tile>>, Vec<Rc<Tile>>) = tiles
        .into_iter()
        .partition(|ref tile| tile.impass.len() == 0);
